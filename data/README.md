...
## The other seed directory

`data/seed/` (singular) holds `family_friendly_seed.csv`: 134 rows of **entirely
fictional** venues generated by `scripts/generate_seed_data.py`. It is not part
of the pipeline above and is never served. It exists so the recommender tests
have a dataset with the columns they exercise — `price_usd`, `min_age`,
`max_age` — and deliberately includes non-US regions (`Greater London`,
`Ontario`) so that logic assuming a two-letter US state code fails in a test
rather than in production.

Do not point `FAMILY_DATASET_URL` at it outside of tests. `data/seeds/`
(plural) is the real source of truth.
